...
public class Matar   : MonoBehaviour {
	...
	void OnTriggerEnter(Collider collision)						// used for things like bullets, which are triggers.  
	{
		if (damageOnTrigger) {
			...
			if (collision.gameObject.GetComponent<Salud> () != null) {	// if the hit object has the Health script on it, deal damage
				collision.gameObject.GetComponent<Salud> ().ApplyDamage (damageAmount);
	...
	{
		if (continuousDamage) {
			if (collision.gameObject.tag == "Player" && collision.gameObject.GetComponent<Salud> () != null) {	// is only triggered if whatever it hits is the player
